...
*   **Endpoint:** `POST /rest/token/auth`
*   **Request Body:** A JSON object containing the user's `username` and `password`.

**Example Request:**

```bash
curl --location 'http://<ILIAS_BASE_URL>/rest/token/auth' \
...
*   **Endpoint:** `POST /rest/token/refresh`
*   **Request Body:** A JSON object containing the `refresh_token`.

**Example Request:**

```bash
curl --location 'http://<ILIAS_BASE_URL>/rest/token/refresh' \
--header 'Content-Type: application/json' \
--data '{
